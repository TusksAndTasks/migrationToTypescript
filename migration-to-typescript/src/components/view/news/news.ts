...
class News {
    draw(data?: mainData[]) {
        ...
        news.forEach((item: mainData, idx: number): void => {
            ...
            if(item.publishedAt){ (<HTMLElement>newsClone.querySelector('.news__meta-date')).textContent = item.publishedAt
                .slice(0, 10)
                .split('-')
                .reverse()
                .join('-');
        ...
        if (item.url) {(<HTMLElement>newsClone.querySelector('.news__read-more a')).setAttribute('href', item.url)};
